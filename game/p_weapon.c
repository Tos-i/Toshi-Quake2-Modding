...
void ChangeWeapon (edict_t *ent)
{
	...
	if (!ent->client->chasetoggle) ent->client->ps.gunindex = gi.modelindex(ent->client->pers.weapon->view_model);
...
void power_fist(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
...
void fist_attack(edict_t* ent, vec3_t g_offset, int damage)
{
	...
	power_fist(ent, start, forward, damage, SWORD_KICK);
}
...
void Weapon_Fist(edict_t* ent)
{
	...
	Weapon_Generic(ent, 4, 12, 50, 54, pause_frames, fire_frames, Weapon_Fist_Fire);
}
...
void combat_knife(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	//You may recognize a lot of this from the fire lead command, which
	//is the one that I understood best what the hell was going on
...
void knife_attack(edict_t* ent, vec3_t g_offset, int damage)
{
	...
	combat_knife(ent, start, forward, damage, SWORD_KICK/4);
}
...
void weapon_supershotgun_fire (edict_t *ent)
{
	...
	fire_shotgun (ent, start, forward, damage, kick, 200, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SSHOTGUN_COUNT/2, MOD_SSHOTGUN);
